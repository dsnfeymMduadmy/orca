/**
 * @file lv_conf.h
 * Конфигурация LVGL v9.5.0 для Orca (STM32H743, 480 МГц).
 *
 * Файл получен из lv_conf_template.h апстрима; ниже перечислено всё, что
 * отличается от шаблона, — чтобы при обновлении LVGL было видно, что
 * переносить:
 *
 *   LV_COLOR_DEPTH 16        RGB565 — формат LTDC-фреймбуфера и orca_fbstream
 *   LV_MEM_ADR               куча LVGL живёт в SDRAM (ORCA_GUI_HEAP_BASE),
 *                            а не во внутренних 512 КБ AXI SRAM
 *   LV_MEM_SIZE 2 MB         ровно ORCA_GUI_HEAP_SIZE
 *   LV_USE_OS FREERTOS       lv_lock/lv_unlock на мьютексе ядра: к дереву
 *                            объектов ходит и задача ui, и модули
 *   LV_DRAW_BUF_ALIGN 32     строка кэша D-Cache: буферы в SDRAM чистятся
 *                            через SCB_CleanDCache_by_Addr, а он работает
 *                            строками по 32 байта
 *   LV_FONT_MONTSERRAT_16/20/28/48
 *                            временные шрифты (латиница+цифры). Кириллицу
 *                            даст свой генератор (tools/fontgen), после чего
 *                            монтсеррат можно выключить
 *   LV_USE_LOG 1             предупреждения и ошибки LVGL уходят в консоль
 *                            устройства (orca_lvgl.c, lv_log_register_print_cb):
 *                            без них исчерпание LV_MEM_SIZE выглядит как
 *                            молча не нарисованный виджет. LV_LOG_PRINTF 0 —
 *                            печатает наш колбэк, с тегом [lvgl]
 *
 * DMA2D (LV_USE_DRAW_DMA2D) пока 0: для него нужен HAL-модуль dma2d и
 * аккуратная работа с кэшем. Включаем отдельным шагом, когда поднимется LTDC.
 */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #define LV_MEM_SIZE (2 * 1024 * 1024U)    /**< [bytes] — весь ORCA_GUI_HEAP */

    /** Size of the memory expand for `lv_malloc()` in bytes */
    #define LV_MEM_POOL_EXPAND_SIZE 0

    /** Set an address for the memory pool instead of allocating it as a normal array. Can be in external SRAM too. */
    #define LV_MEM_ADR 0xC0400000UL   /**< ORCA_GUI_HEAP_BASE, см. orca_memmap.h */
